...
# Tools
RUN apt install aircrack-ng crackmapexec crunch \ 
                curl dirb dirbuster dnsenum dnsrecon \ 
                dnsutils dos2unix enum4linux exploitdb ftp \ 
                git gobuster hashcat hping3 hydra impacket-scripts \ 
                john joomscan masscan metasploit-framework mimikatz \ 
                nasm ncat netcat-traditional nikto nmap patator \ 
                php powersploit proxychains \ 
                python3-impacket python-pip python2 python3 \
                recon-ng responder samba samdump2 smbclient smbmap snmp \ 
                socat sqlmap sslscan sslstrip theharvester vim \ 
                wafw00f weevely wfuzz whois wordlists wpscan \ 
